...
contract JioWhitelistSale is Ownable, Pausable {
    using SafeERC20 for IERC20;
    IERC20 public immutable buyingToken;
    IERC20 public immutable jioToken;
    ...
    function withdrawBuyingTokens() public onlyOwner {
        buyingToken.safeTransfer(
            msg.sender,
            buyingToken.balanceOf(address(this))
        );
    }

    function buyingTokenAmountToJioAmount(
    ...
    function participate(
        ...
        uint256 buyingAmountToJioAmount = buyingTokenAmountToJioAmount(
            buyingTokenAmount
        );
        require(
            totalJioSold + buyingAmountToJioAmount <= maxJioToSell,
            "sold out!"
        );

        buyingToken.safeTransferFrom(
            msg.sender,
            address(this),
            buyingTokenAmount
        );
    ...
    function claim(uint256 purchaseId) public {
        ...
        require(
            purchase.purchaseTime + MAX_LOCK_PERIOD < block.timestamp,
            "cant withdraw early"
        );
